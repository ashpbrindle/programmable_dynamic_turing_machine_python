...
class TuringMachineGUI():
    def __init__(self, window):
        ...
        self.tape_options = [
            "Empty Tape",
            "[0, 0, 0] (Odd number of 0's (3))",
            "[0, 0, 0, 0] (Even Number of 0's (4) [Will cause an infinite loop])"
        ]

        # dictionary holding all of the tapes associated with each option
        self.tapes = {
            "Empty Tape": ["#", "#", "#", "#", "#", "#", "#", "#", "#", "#"],
            "[0, 0, 0] (Odd number of 0's (3))": ["0", "0", "0"],
            "[0, 0, 0, 0] (Even Number of 0's (4) [Will cause an infinite loop])": ["0", "0", "0", "0"]
        }

        # a dictionary handling each starting state asociated with each option
        self.starting_states = {
            "Empty Tape": "A",
            "[0, 0, 0] (Odd number of 0's (3))": "A",
            "[0, 0, 0, 0] (Even Number of 0's (4) [Will cause an infinite loop])": "A"
        }

        # a dictionary to handle the starting positions of each option
        self.tape_positions = {
            "Empty Tape": 0,
            "[0, 0, 0] (Odd number of 0's (3))": 0,
            "[0, 0, 0, 0] (Even Number of 0's (4) [Will cause an infinite loop])": 0
        }
    ...
    def drawWindow(self):
        ...
        self.photoState = PhotoImage(file = "TM_odd_number_0s_State_Diagram_small.gif")
        self.imgStates.create_image(0,0, image = self.photoState, anchor="nw")
        self.imgStates.place(x = 750, y = 80)
...
class TuringStateB(State, Transition):
    ...
    def foundBlank(self):
        ...
        print "(R) Direction"
        app.lblDirection.config(text = "R")
        root.update()
        sleep(0.2)
